...
import { HTTPStatus, AuthService } from '@front-application/core';
import { Subject, Subscription, takeUntil, timer, take } from 'rxjs';
  ...
  resetTime(idleTime: number = this.idleTime) {
    ...
    this.countdown = timer(0, 1000).pipe(take(duration)).subscribe(
      ...
      value => { }, err => { }, () => {
        ...
        location.href = `${window.location.origin}/auth/login?returnUrl=${window.location.pathname}`;
      })
  }
}
